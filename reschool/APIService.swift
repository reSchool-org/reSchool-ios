...
class APIService: ObservableObject {
    ...
    func login(username: String, password: String, rememberMe: Bool = false) async throws -> Bool {
        ...
        let deviceId = CryptoHelper.randomString(length: 32)
        let pushToken = CryptoHelper.randomString(length: 64)

        let devicePayload = DevicePayload(
            cliType: "web", cliVer: "v.2515", pushToken: pushToken,
            deviceId: deviceId, deviceName: "iOS App", deviceModel: 120, cliOs: "iOS"
        )
